...
if (Brand == 1){
	...
	if (Brand == 6) {
		V_Bus.setFIFOFilter(0, 0x18FF8F23, EXT);  //McCormick Wheel Angle only
	...
	if (Brand == 0) {
		// lansalot
		K_Bus.setFIFOFilter(0, 0x10374d4c, EXT);  // Claas MR Arion engage
	...
	if (Brand == 2) {
		...
		K_Bus.setFIFOFilter(1, 0x18FFB031, EXT);  //CaseIH Engage Message alt 1
		K_Bus.setFIFOFilter(2, 0x18FFB306, EXT);  //CaseIH Engage Message alt 2
		K_Bus.setFIFOFilter(3, 0x18FE4523, EXT);  //CaseIH Rear Hitch Infomation
	}
	...
	if (V_Bus.read(VBusReceiveData)) {
		...
		if (Brand == 0)
		{
			...
			if (VBusReceiveData.id == 0x18EF1CD2)
			{
				...
				if ((VBusReceiveData.buf[0]) == 0x27) //&& (VBusReceiveData.buf[2]) == 241)   //Ryan MR Models? Remove databuf2 read for stage5 new ATP software
				{
					Serial.print("vbus. ");
					Serial.println(VBusReceiveData.buf[1]);
					Serial.println(millis());
					if (millis() - lastCANCommand > 1000 && VBusReceiveData.buf[1] == 1) {
		...
		{
			//**Current Wheel Angle & Valve State**
			if (VBusReceiveData.id == 0x18Ff8F23)
			{
				estCurve = ((VBusReceiveData.buf[4] << 8) + VBusReceiveData.buf[3]);  // CAN Buf[1]*256 + CAN Buf[0] = CAN Est Curve 
				steeringValveReady = (VBusReceiveData.buf[2]);
			}
...
void K_Receive()
{
	...
	if (K_Bus.read(KBusReceiveData)) {
		...
		if (Brand == 0) { // Claas MR engage button (lansalot
			if (KBusReceiveData.id == 0x10374D4C) {
				if (millis() - lastCANCommand > 1000 && KBusReceiveData.buf[1] & 0x10) {
					lastCANCommand = millis();
					if (intendToSteer == 0) {
						Time = millis();
						digitalWrite(engageLED, HIGH);
		...
		if (Brand == 2) { // Case Engage buttons
			if (KBusReceiveData.id == 0x18FFB306 || KBusReceiveData.id == 0x18FFB031) {
				if (millis() - lastCANCommand > 1000 && KBusReceiveData.buf[2] & 0x10) {
					lastCANCommand = millis();
					if (intendToSteer == 0) {
						Time = millis();
						digitalWrite(engageLED, HIGH);
			...
			if (KBusReceiveData.id == 0x14FF7706) {
				if (millis() - lastCANCommand > 1000 && (KBusReceiveData.buf[0] & 0x82 || KBusReceiveData.buf[0] & 0xB2 || KBusReceiveData.buf[0] & 0xC1 || KBusReceiveData.buf[0] & 0xC4)) {
					lastCANCommand = millis();
					if (intendToSteer == 0) {
						Time = millis();
						digitalWrite(engageLED, HIGH);
