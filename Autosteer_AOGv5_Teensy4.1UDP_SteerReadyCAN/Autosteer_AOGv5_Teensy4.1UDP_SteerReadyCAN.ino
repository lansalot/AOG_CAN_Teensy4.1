...
void udpSteerRecv(int sizeToRead)
{
	...
	if (udpData[0] == 0x80 && udpData[1] == 0x81 && udpData[2] == 0x7F) //Data
	{
		...
		else if (udpData[3] == 0x64)  //Corrected GPS Data snarfed to ISOBUS
		{
			if (sendGPStoISOBUS)
			{
				if (GPS_5hz > 4)
				{
					...
					memcpy(&correctedLon, &udpData[5], sizeof(correctedLon));
					memcpy(&correctedLat, &udpData[13], sizeof(correctedLat));

					sendISOBUS_65267_65256(correctedLat, correctedLon);
				}
